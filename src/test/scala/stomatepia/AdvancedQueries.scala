...
class AdvancedQueries extends FunSuite {
  ...
  implicit def isCursor(c:Cursor) = new {
    def is(s:String) = assert(c.toString === s)
  }
  ...
  test("intro"){
    ...
    db.things.find(_.j $ne 3, _.k $gt 10)    is """db.things.find({ "j" : { "$ne" : 3 }, "k" : { "$gt" : 10 } })"""

    db.things.insert(_.colors(Seq("blue", "black")))

    db.things.insert(_.colors(Seq("yellow", "orange", "red")))

    db.things.find(_.colors $ne "red")       is """db.things.find({ "colors" : { "$ne" : 'red' } })"""
  }
  ...
  test("<,<=,>,>="){
    ...
    db.collection.find(_.field $gt value)  is """db.collection.find({ "field" : { "$gt" : 1 } })"""
    db.collection.find(_.field > value)    is """db.collection.find({ "field" : { "$gt" : 1 } })"""

    db.collection.find(_.field $lt value)  is """db.collection.find({ "field" : { "$lt" : 1 } })"""
    db.collection.find(_.field < value)    is """db.collection.find({ "field" : { "$lt" : 1 } })"""

    db.collection.find(_.field $gte value) is """db.collection.find({ "field" : { "$gte" : 1 } })"""
    db.collection.find(_.field >= value)   is """db.collection.find({ "field" : { "$gte" : 1 } })"""

    db.collection.find(_.field $lte value) is """db.collection.find({ "field" : { "$lte" : 1 } })"""
    db.collection.find(_.field <= value)   is """db.collection.find({ "field" : { "$lte" : 1 } })"""
  }

  test("$all"){
    db.things.find(_.a $all Seq(2, 3))    is """db.things.find({ "a" : { "$all" : [ 2, 3 ] } })"""

    db.things.find(_.a $all Seq(2, 3, 4)) is """db.things.find({ "a" : { "$all" : [ 2, 3, 4 ] } })"""
  }

  test("$exists"){
    db.things.find(_.a $exists true)      is """db.things.find({ "a" : { "$exists" : true } })"""

    db.things.find(_.a $exists false)     is """db.things.find({ "a" : { "$exists" : false } })"""
  }

  test("$mod"){
    db.things.find("this.a % 10 == 1")    is """db.things.find('this.a % 10 == 1')"""

    db.things.find(_.a $mod (10, 1))      is """db.things.find({ "a" : { "$mod" : [ 10, 1 ] } })"""
  }

  test("$ne"){
    db.things.find(_.x $ne 3)             is """db.things.find({ "x" : { "$ne" : 3 } })"""
  }

  test("$in"){
    val array = Seq(1, 2, 3)
    db.collection.find(_.field $in array) is """db.collection.find({ "field" : { "$in" : [ 1, 2, 3 ] } })"""

    db.things.find(_.j $in Seq(2, 4, 6))  is """db.things.find({ "j" : { "$in" : [ 2, 4, 6 ] } })"""
  }

  test("$nin"){
    db.things.find(_.j $nin Seq(2, 4, 6)) is """db.things.find({ "j" : { "$nin" : [ 2, 4, 6 ] } })"""
  }
  ...
  test("$and"){
    ...
    db.foo.find(_.$and(_.a === 1, _.a $gt 5)) is """db.foo.find({ "$and" : [ { "a" : 1 }, { "a" : { "$gt" : 5 } } ] })"""
  }

  test("$size"){
    db.things.find(_.a $size 1) is """db.things.find({ "a" : { "$size" : 1 } })"""
  }

  test("$type"){
    db.things.find(_.a $type 2)  is """db.things.find({ "a" : { "$type" : 2 } })"""
    db.things.find(_.a $type 16) is """db.things.find({ "a" : { "$type" : 16 } })"""
  ...
  test("$elemMatch"){
    db.t.find(_.x(_.$elemMatch (_.a === 1, _.b $gt 1))) is """db.t.find({ "x" : { "$elemMatch" : { "a" : 1, "b" : { "$gt" : 1 } } } })"""

    db.t.find(_.x.a === 1, _.x.b $gt 1)              is """db.t.find({ "x.a" : 1, "x.b" : { "$gt" : 1 } })"""
  ...
  test("Meta operator: $not"){
    ...
    db.things.find(_.a $not(_.$mod(10, 1)))                                 is """db.things.find({ "a" : "$not" : { "$mod" : [ 10, 1 ] } })"""

//    db.things.find(_.a $not true) -- syntax error
  }

  test("Javascript expressions and $where"){
    db.myCollection.find(_.a $gt 3)                                   is """db.myCollection.find({ "a" : { "$gt" : 3 } })"""
